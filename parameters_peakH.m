...
H_peak=zeros(Nscan,1); %initialize vector to store peak uninsured hospitalizations
 
for jj=1:Nscan %add a loop to run the simulation for each value
   
  c_i=c_i_vals(jj); %update the value of c_u
...
figure;
plot(c_i_vals,H_peak,'o-')
xlabel('c_i')
ylabel('H_peak')
 
figure;
plot(c_u_vals,H_peak,'o-')
xlabel('c_u')
ylabel('H_peak')
