...
%----------plotting R_0 for different beta values-----------------------%
...
for jj=1:Nscan %add a loop to run the simulation for each value
    ...
    R0_values(jj)=(beta*S_u_0/N)/(delta_u*(1-c_u)+gamma_u*c_u) + (beta*S_i_0/N)/(delta_i*(1-c_i)+gamma_i*c_i);  %get max of 5th column of y, which stores H_u
   %myplot(t, y, t0, tf)
end
