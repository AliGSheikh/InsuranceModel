...
%c_i_vals=linspace(0.03,0.05,Nscan); %set up a range of values for c_u to scan over
...
for jj=1:Nscan %add a loop to run the simulation for each value
  ...
  %H_peak(jj)=max(y(:,6)+y(:,5)); %get max of 5th column of y, which stores H_u
  H_peak(jj)=max(y(:,10)+y(:,9));
