...
d_i = 0.225;
d_u=(k-p_i*d_i)/p_u
%d_u = 0.24; % probability that uninsured ICU patient will die % we will need to find a systematic way to determine this
 % make sure d_u > d_i
...
d_i_vals=linspace(0.17,0.23,Nscan); %set up a range of values for c_u to scan over
c_i_vals=linspace(0.03,0.05,12); %set up a range of values for c_u to scan over
H_peak=zeros(Nscan,Nscan); %initialize vector to store peak uninsured hospitalizations
 for kk=1:12
     c_i=c_i_vals(kk);
     c_u = (h - p_i*c_i)/p_u
     c_u_vals(kk)=c_u;
for jj=1:Nscan %add a loop to run the simulation for each value
  d_i=d_i_vals(jj); %update the value of c_i
  d_u=(k-p_i*d_i)/p_u
  %c_u = (h - p_i*c_i)/p_u % varying c_i but avg of hosp rate is the same
  d_u_vals(jj)=d_u;
  ...
  H_peak(jj,kk)=max(y(:,6)+y(:,5)); %get max of 5th column of y, which stores H_u
  %H_peak(jj,kk)=max(y(:,10)+y(:,9));
  %H_peak(jj)=(y(end,10)+y(end,9)); %take death at the end and add in and un
  %myplot(t, y, t0, tf)
end
 end
 
 contourf(c_i_vals,d_i_vals,H_peak,'o-')
 colorbar()
%plot the peak hospitalization of uninsured for the different values of c_u
%figure;
%plot(d_i_vals,H_peak,'o-')
%xlabel('d_i')
%ylabel('H_peak')
%figure;
%plot(d_u_vals,H_peak,'o-')
%xlabel('d_u')
%ylabel('H_peak')
%%
...
figure(1) %can take out 1 to see other graphs
subplot(5,2,1)
