...
N = S_u_0 + S_i_0 + I_i_0 + I_u_0; % total population remains constant


%-----Let's solve this thing!
[t,y] = ode45(@(t,y) sihr(t, y, N, d_u, d_i, c_u, c_i, alpha_u, alpha_i, delta_u, delta_i, gamma_u, gamma_i, ksi_u, ksi_i, daily_unemployment_vec, eta, unemployment_factor, time_varying_beta), tee, [S_u_0, S_i_0, I_u_0, I_i_0, R_u_0, R_i_0]); 
...
aprime = [-beta * S_u * I / N + l * S_i - g * S_u; % dS_u/dt
    ...
    beta * S_u * I / N - (gamma_u * c_u * I_u)*(ksi_u*d_u + alpha_u*(1-d_u)) - delta_u * (1-c_u) * I_u; % dI_u/dt
    beta * S_i * I / N - (gamma_i * c_i * I_i)*(ksi_i*d_i + alpha_i*(1-d_i)) - delta_i * (1-c_i) * I_i; % dI_i/dt
    %gamma_u * c_u * I_u - (ksi_u * d_u * H_u) - alpha_u * (1 - d_u) * H_u; % dH_u/dt
    %gamma_i * c_i * I_i - (ksi_i * d_i * H_i) - alpha_i * (1 - d_i) * H_i; % dH_i/dt
    delta_u * (1-c_u) * I_u + alpha_u * (1 - d_u) * (gamma_u * c_u * I_u); % dR_u/dt
    delta_i * (1-c_i) * I_i + alpha_i * (1 - d_i) * (gamma_i * c_i * I_i);]; % dR_i/dt
